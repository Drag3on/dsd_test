...
class Scale_UART:
    ...
    def su_fc_control(self, F, W, B, R, vaddr, faddr):
        ...
        dataa = self.su_read_data(faddr + 0x700)
        print(dataa)
        print("3")
        ...
        self.su_write_data(faddr + 0x00, 0x6)
        while (True):
            done = self.su_read_data(faddr + 0xc)#fc_done       
            print(done)
            dataa = self.su_read_data(faddr + 0x100)#states
            print(dataa)
            dataa = self.su_read_data(faddr + 0x200)#bram_addr
            print(dataa)
            dataa = self.su_read_data(faddr + 0x300)#fc_start
            print(dataa)
            dataa = self.su_read_data(faddr + 0x400)#F_B_W_cal_done
            print(dataa)
            dataa = self.su_read_data(faddr + 0x500)#input_size
            print(dataa)
            dataa = self.su_read_data(faddr + 0x600)#output_size
            print(dataa)
    ...
    def su_conv_control(self, I, F, W, B, R, vaddr, caddr):
        ...
        self.su_write_data(caddr + 0x00, 0x4) 
        ...
        while (True):
            done = self.su_read_data(caddr + 0x4)
            if (int.from_bytes(done, 'big', signed=True) == 1):
                break
...
####################### RESULT CHECK
    ...
    def check_timing(self, name, module_addr):
        ...
        print(name + "clk elapsed: %d" % (clk_cnt))
